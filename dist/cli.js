...
var perfRules = [
  {
    ...
    async check(file) {
      ...
      return /for\s*\(.*\)\s*{[\s\S]*await/.test(content) ? [{ type: "PERF", file, msg: "await inside loop" }] : [];
    }
  }
];
...
var securityRules = [
  {
    ...
    async check(file) {
      ...
      return /(API_KEY|SECRET|TOKEN)\s*=\s*['"]/i.test(content) ? [{ type: "SEC", file, msg: "Hardcoded secret detected" }] : [];
    }
  }
];
...
`).filter((line) => line.trim() !== "").map((line) => ({ type: "TYPE", file: "ts", msg: line }));
        return errors;
      }
    }
  }
...
function report(issues) {
  if (!issues.length)
    return console.log(`\u2705 Code is optimized and safe!
`);
...
// src/engine.ts
async function runOptimizeGuard() {
...
// src/cli.ts
var result = await runOptimizeGuard();
if (!result.ok)
  process.exit(1);
